...
const TYPE_CONFIG = {
  success: { color: '#a855f7', Icon: CheckCircle },
  ...
  return (
    <div
      style={{
        ...
        boxShadow:    `0 8px 32px rgba(0,0,0,0.6), 0 0 20px ${cfg.color}15`,
      }}
    >
      <cfg.Icon size={18} color={cfg.color} style={{ flexShrink: 0, marginTop: 1 }} />
      <div style={{ flex: 1, minWidth: 0 }}>
        <div style={{ fontFamily: 'var(--font-body)', fontWeight: 600, fontSize: '14px', color: '#ede9ff' }}>
          {t.title}
        </div>
        {t.body && (
          <div style={{ fontFamily: 'var(--font-body)', fontSize: '12px', color: '#9985c8', marginTop: '4px' }}>
            {t.body}
          </div>
        )}
        {t.txHash && (
          <div style={{ marginTop: '8px', display: 'flex', alignItems: 'center', gap: '6px' }}>
            <span style={{ fontFamily: 'var(--font-mono)', fontSize: '11px', color: '#4a3d70' }}>
              {t.txHash.slice(0, 14)}…
            </span>
            <a
              ...
              style={{ display: 'inline-flex', alignItems: 'center', gap: '3px', color: '#a855f7', fontSize: '11px' }}
            >
      ...
      <button
        onClick={dismiss}
        style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#4a3d70', padding: 0, flexShrink: 0 }}
      >
        <X size={16} />
      </button>
    </div>
