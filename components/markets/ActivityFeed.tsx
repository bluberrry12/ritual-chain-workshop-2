...
export function ActivityFeed({ marketId }: { marketId: bigint }) {
  ...
  if (entries.length === 0) {
    return (
      <div style={{ fontFamily: 'var(--font-body)', fontSize: '13px', color: '#4a3d70', padding: '16px 0' }}>
        No activity yet.
      </div>
    )
  }
        ...
        {entries.map(e => (
          ...
          >
            <span style={{
              ...
              background: e.isYes ? '#22d3ee' : '#f43f5e',
            }} />
            <span style={{ color: '#9985c8' }}>{e.addr}</span>
            <span style={{ color: '#4a3d70' }}>→ stake</span>
            <span style={{ color: '#ede9ff' }}>{formatRitual(e.amount, 3)} RITUAL</span>
            <span style={{ color: e.isYes ? '#22d3ee' : '#f43f5e', fontWeight: 700 }}>
              {e.isYes ? 'YES' : 'NO'}
            </span>
            <span style={{ color: '#352d64', marginLeft: 'auto' }}>· block {e.block.toLocaleString()}</span>
          </motion.div>
        ))}
      </AnimatePresence>
    </div>
