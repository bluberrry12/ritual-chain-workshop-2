  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
      <div style={{
        ...
        height, background: '#221e40',
      }}>
        <motion.div
          animate={{ width: `${pct}%` }}
          transition={{ duration: 0.4, ease: 'easeInOut' }}
          style={{ background: empty ? '#352d64' : '#22d3ee', height: '100%', minWidth: 0 }}
        />
        <motion.div
          animate={{ width: `${100 - pct}%` }}
          transition={{ duration: 0.4, ease: 'easeInOut' }}
          style={{ background: empty ? '#352d64' : '#f43f5e', height: '100%', minWidth: 0 }}
        />
      </div>
      {showLabels && (
        <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '11px', fontFamily: 'var(--font-mono)' }}>
          <span style={{ color: '#22d3ee' }}>{pct >= 5 ? `YES ${pct.toFixed(1)}%` : ''}</span>
